	    ...
	    select: function(event, ui) {
	    	...
	    	$.ajax({
				...
				success: function (xhr, result)
				{
					...
					$hidden = '<input type="hidden" name="workers[]" value="' + xhr.id + '" />';
					$color = 'border-secondary text-dark';
					if(xhr.suspend == 1) {
						$color = 'border-danger text-danger';
						$hidden = '';
					}
					$child = '<div class="everyWorker col-4 border ' + $color + '"><div class="row"><div class="col-5"><img src="/img/avatar/' + xhr.avatar + '" class="img-thumbnail mt-3"></div><div class="col-7"><br>會員編號 : <br>' + xhr.m_no + '<br>' + xhr.nickname + '<br>' + xhr.phone + '<br>' + $hidden + '<button class="btn btn-danger float-right deleteMember mb-3" type="button">取消</button></div></div></div>';
	                $box.append($child);
